...
def plot_learning_curve(results, title):
    ...
    plt.tight_layout()
    plt.savefig(f'original_curves/{title.replace(" ", "_")}_learning_curve.png')
    plt.close()
...
def main():
    ...
    # 优化器 - 使用Adam优化器，添加权重衰减
    optimizer = optim.Adam(softmax_model.parameters(), lr=0.001, weight_decay=1e-4)
    # 训练模型，增加训练轮数
    softmax_results = train(softmax_model, train_loader, val_loader, criterion, optimizer, num_epochs=20)
    # 绘制学习曲线
    ...
    criterion = nn.CrossEntropyLoss()
    # 优化器 - 使用Adam优化器，添加权重衰减
    optimizer = optim.Adam(fc_model.parameters(), lr=0.001, weight_decay=1e-4)
    # 训练模型，增加训练轮数
    fc_results = train(fc_model, train_loader, val_loader, criterion, optimizer, num_epochs=20)
    # 绘制学习曲线
    ...
    # 损失函数
    criterion = nn.CrossEntropyLoss()
    # 优化器 - 使用SGD优化器，添加权重衰减和学习率衰减
    optimizer = optim.SGD(fc_sgd_model.parameters(), lr=0.01, momentum=0.9, weight_decay=1e-4)
    # 训练模型，增加训练轮数
    fc_sgd_results = train(fc_sgd_model, train_loader, val_loader, criterion, optimizer, num_epochs=20)
    # 绘制学习曲线
